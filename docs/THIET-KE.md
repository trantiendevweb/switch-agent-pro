...
|---|---|---|
| Nền | `#0F172A` (tránh `#000000` — smear OLED) | canvas 3D |
| Nền phụ/muted | `#1E293B` / `#272F42` | panel kính mờ |
| Chữ | `#F8FAFC` | nhãn |
| **Accent — đang chạy** | `#22C55E` (xanh "run") | orb hoạt động, gauge ok |
| Cảnh báo/hết hạn mức | `#EF4444` | orb đỏ |
| Viền | `#475569` | khung panel |
| Font | **Inter** 300–700 | toàn bộ |
| Easing | `expo.out` — `cubic-bezier(0.16,1,0.3,1)` | chuyển cảnh |
| Hiệu ứng | glassmorphism (blur ~20), ambient light blobs, scale press 0.97→1 | panel + orb |
...
- Mỗi **profile/phiên** = một orb; đập/sáng theo hoạt động; màu theo trạng thái
  (`#22C55E` chạy / muted nghỉ / `#EF4444` hết hạn mức·lỗi).
- **Hạn mức/token** = kích thước·độ cao orb hoặc gauge — thấy ngay phiên nào sắp cạn quota.
- **Kết nối** = đường nối orchestrator ↔ các phiên; sáng lên khi có luồng.
...
- **`FogExp2`** (`0x0a0a0a`, density ~0.02): chiều sâu khí quyển + cull ngầm orb ở xa.
...
**Về skill:** "UXUI" chính là skill **`ui-ux-pro-max`** bạn đã gửi (có sẵn stack
`threejs`, `react`, `nextjs`; bộ 192 palette, 98 hướng dẫn UX, 25 loại biểu đồ).
